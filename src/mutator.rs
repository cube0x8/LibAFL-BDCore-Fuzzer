...
const BERIA_ANCHOR_OFF: usize = 0x181248;
const BERIA_WORKBUF_SIZE: usize = 0x184000;
    ...
    S: HasRand + HasMetadata + HasCorpus<BytesInput> + HasSolutions<BytesInput> + HasMaxSize,
{
    fn mutate(&mut self, state: &mut S, input: &mut BytesInput) -> Result<MutationResult, Error> {
        ...
        if bytes.len() != BERIA_WORKBUF_SIZE || !in_bounds(bytes, BERIA_ANCHOR_OFF, 4) {
            return Ok(MutationResult::Skipped);
        }

        let anchor = match read_u32_le(bytes, BERIA_ANCHOR_OFF) {
            Some(v) => v,
            None => return Ok(MutationResult::Skipped),
        };
        let v7 = anchor.wrapping_sub(BERIA_IMAGE_BASE) as usize;

        if !in_bounds(bytes, v7 + 0x18, 4) {
            return Ok(MutationResult::Skipped);
        }

        let v8 = match read_u32_le(bytes, v7 + 0x00) {
            Some(v) => v.wrapping_sub(BERIA_IMAGE_BASE) as usize,
            None => return Ok(MutationResult::Skipped),
        };
        let v12 = match (read_u32_le(bytes, v7 + 0x04), read_u32_le(bytes, v7 + 0x0c)) {
            (Some(a), Some(b)) => a.wrapping_sub(b) as usize,
            _ => return Ok(MutationResult::Skipped),
        };

        if !in_bounds(bytes, v8 + 0x10, 1) || !in_bounds(bytes, v12, 4) {
            return Ok(MutationResult::Skipped);
        }
        ...
        match op {
            ...
            0 => {
                ...
                if new_v12 + 4 >= bytes.len() || new_v8 + 0x110 >= bytes.len() {
                    return Ok(MutationResult::Skipped);
                }
                ...
                write_u32_le(bytes, new_v7 + 0x0c, BERIA_IMAGE_BASE);
                write_u32_le(
                    bytes,
                    new_v7 + 0x10,
                ...
                write_u32_le(bytes, new_v12, choose_u32(state, &[0, 0, 0, 1, 2, 4]));
                write_u32_le(bytes, new_v8 + 0x00, choose_u32(state, &[1, 1, 1, 2, 3]));
            }
            ...
            5 => {
                self.mutate_dword(state, bytes, v12, &[0, 0, 0, 1, 2, 4, 8, 0x10, 0x20]);
            }
            ...
            // flags
            9 => {
                self.mutate_dword(state, bytes, v8 + 0x0c, &[0, 1, 2, 0xff, 0xffffffff]);
            }

            // payload bytes at v8 + 0x10
            _ => {
                self.mutate_payload(state, bytes, v8 + 0x10, 0x100);
            }
        }
