...
use super::{
    BeriaWorkbufMutator, FixedSizeHavocMutationsType, MorphinepStreamMutator, PelockStubMutator,
    UpackWindowMutator,
};
    ...
    S: HasRand + HasMetadata + HasCorpus<BytesInput> + HasSolutions<BytesInput> + HasMaxSize,
{
    fn mutate(&mut self, state: &mut S, input: &mut BytesInput) -> Result<MutationResult, Error> {
        match self {
            ...
            Self::Pelock(m) => m.mutate(state, input),
    ...
    fn post_exec(&mut self, state: &mut S, new_corpus_id: Option<CorpusId>) -> Result<(), Error> {
        match self {
            ...
            Self::Pelock(m) => m.post_exec(state, new_corpus_id),
