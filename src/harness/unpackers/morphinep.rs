...
impl CevaTarget for MorphinepTarget {
    ...
    fn initialize(
        ...
        if harness
            .qemu()
            .read_mem(pe_addr + 0x138, &mut pe_ptr_buf)
            .is_ok()
        {
            ...
            if harness
                .qemu()
                .read_mem(pe_ptr + 0x0c, &mut addend_buf)
                .is_ok()
            {
        ...
        if harness
            .qemu()
            .read_mem(entry_buf_addr, &mut entry_buf)
            .is_ok()
        {
            log::info!(
    ...
    fn prepare_input(&self, _qemu: &Qemu, input: &[u8], input_len: GuestReg) -> Result<(), Error> {
        ...
        let (entry_patch, stream_input) =
            if final_input_len >= MORPHINEP_ENTRYBUF_LEN + Self::LEGACY_INPUT_LEN {
        ...
        if !entry_patch.is_empty() {
            _qemu
                ...
                .map_err(|e| {
                    Error::unknown(format!("Failed to patch Morphinep entry buffer: {e:?}"))
                })?;
        }
        ...
        if self.dispatch_probe_enabled.get() {
            ...
            if let Some(label) = label {
                ...
                qemu.write_reg(Regs::Pc, GuestReg::try_from(harness.exit_point).unwrap())
                    .map_err(|e| {
                        Error::unknown(format!("Failed to redirect PC to exit point: {e:?}"))
                    })?;
                return Ok(true);
            }
        }
