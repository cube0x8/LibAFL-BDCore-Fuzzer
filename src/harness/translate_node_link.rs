...
impl CevaTarget for TranslateNodeLinkTarget {
    ...
    fn prepare_input(
        ...
        qemu.read_mem(emulator_node + 0x28, &mut guest_bytes).unwrap();
        let guest_bytes_ptr: GuestAddr = u64::from_le_bytes(guest_bytes.try_into().unwrap()).try_into().unwrap();

        let mut guest_bytes_start_addr = [0u8; 8];
        let mut guest_bytes_end_addr = [0u8; 8];
        qemu.read_mem(emulator_node + 0x0, &mut guest_bytes_start_addr).unwrap();
        qemu.read_mem(emulator_node + 0x8, &mut guest_bytes_end_addr).unwrap();
